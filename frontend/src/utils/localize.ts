...
export function formatLocalizedContent(
  ...
  // Prefer the listing-specific translation generated by the backend. This is
  // the authoritative translation for seller-entered content.
  const cached = field ? translations?.[lang]?.[field] : undefined
  if (cached && cached.trim() && cached.trim().toLocaleLowerCase() !== value.trim().toLocaleLowerCase()) {
    return cached
  }

  // Then use the comprehensive dictionary for older records that predate the
  // translation cache or for content that has not yet been backfilled.
  const dynamic = formatDynamicText(value, lang)
...
export function formatProductName(name: string, language: string, translations?: GenericContentTranslations): string {
  ...
  // Seller/product-specific translations always win over generic crop aliases.
  const cached = translations?.[lang]?.name
  if (cached && cached.trim() && cached.trim().toLocaleLowerCase() !== trimmed.toLocaleLowerCase()) {
    return cached
  }

  const cropFormatted = formatCropName(trimmed, lang)
  if (cropFormatted && cropFormatted !== trimmed) return cropFormatted

  return formatLocalizedContent(trimmed, lang, translations, 'name')
}
